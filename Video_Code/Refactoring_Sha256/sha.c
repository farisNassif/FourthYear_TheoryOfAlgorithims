...
/* 
    Definition of a word as per the specification
    Section 2.1
*/
...
/*
    [SIG0(sig_zero) Function]
    =>  TODO
    =>  Section 4.1.2
*/
#define sig_zero(x) (ROTR(x,  7) ^ ROTR(x, 18) ^ SHR(x, 3))

/*
    [SIG1(sig_one) Function]
    =>  TODO
    =>  Section 4.1.2
*/
#define sig_one(x) (ROTR(x, 17) ^ ROTR(x, 19) ^ SHR(x, 10))
...
void nexthash(WORD *M, WORD *H) {
  ...
  WORD W[64];
  WORD a, b, c, d, e, f, g, h, T1, T2; // Should become what current H[] is
  int t;
